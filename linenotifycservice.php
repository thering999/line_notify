...
//$sql_ins = "TRUNCATE TABLE u_count_line_notify;";
 ...
 $sql_ins = "insert into u_count_line_notify(SELECT NOW() DDATE,'7' DAYS,a.ampurcodefull,a.ampurname,count(hospcode) RS,GROUP_CONCAT(x.hospcode ORDER BY x.hospcode) hospcodezero
			...
			WHERE date_serv BETWEEN DATE_SUB(CURDATE(),INTERVAL 8 DAY) AND DATE_FORMAT(NOW(),'%Y-%m-%d') 
			...
			WHERE x.ss < 8 GROUP BY a.ampurcodefull);";

 $rs_ins = mysql_query($sql_ins);
	        if($rs_ins){		
			$sql_line = "SELECT concat(ampurname,'มี ',rs,' หน่วยบริการ : ',hospzero) hzero FROM u_count_line_notify where days = 7 and DATE_FORMAT(ddate,'%Y%m%d') = DATE_FORMAT(NOW(),'%Y%m%d')";
			...
			$txt = "auto: สวัสดีครับ ขณะนี้ ".$table." ยังไม่ได้ส่งข้อมูลแฟ้ม  service ติดต่อกันเป็นเวลา 7 วันหรือต่ำกว่า 5 record ภายใน 7 วัน  ตรวจสอบได้ที่ https://mdh.hdc.moph.go.th/hdc/admin/43file_daily.php";
			line_text($txt);
			}else{
				//echo $sql_ins;
			}
